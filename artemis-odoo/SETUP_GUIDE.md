# Artemis Lab - Odoo Setup & Deployment Guide
...
This is a **real software company** built on Odoo Community + Docker. It manages:
- **Interns** as employees with roles
...
│   ├── addons/
│   │   └── artemis_custom/         # Custom module (all Artemis features)
│   │       ├── __manifest__.py
│   │       ├── __init__.py
│   │       ├── models/
...
docker ps
# Expected output:
# artemis_odoo (running)
# artemis_postgres (running)
```
...
1. **Employees** (`hr`) - For employee/intern records
...
## 4. Activate Custom Module (artemis_custom)

1. **Developer Mode**: Login as admin → bottom-right corner → Enable "Developer Mode"
2. **Go to Apps** → Search `artemis`
3. **Install** `Artemis Custom Module`
...
## 6. Employee Setup (Interns)

Each intern = employee record

**HR** → **Employees** → **Create**

Fill:
- **Name**: Intern name
- **Role Tag**: Select from dropdown (Intern/Developer/etc.)
- **Batch**: 2024/2025/2026
- **GitHub Username**: intern's GitHub handle
...
**Settings**:
- GitHub Repo: `https://github.com/artemis-lab/batch-2025`
- Documentation Status: Not Started

**Add team members**: Select all batch 2025 interns
...
These run automatically - **NO manual configuration needed** (already in artemis_custom).
...
**Permission**: CEO only can delete. Interns: read + upload in own folder.
...
0 2 * * * docker exec artemis_postgres pg_dump -U odoo postgres > /path/to/artemis-odoo/backups/db_$(date +\%Y\%m\%d).sql
```

### Manual Full Backup

```bash
docker exec artemis_postgres pg_dump -U odoo postgres > backups/db_backup_$(date +%Y%m%d_%H%M%S).sql
```
...
# Restore database
docker exec -i artemis_postgres psql -U odoo postgres < backups/db_backup_YYYYMMDD.sql
...
docker exec artemis_postgres pg_dump -U odoo postgres > backups/db_quarter_end_$(date +%Y%m%d).sql

# 2. Delete records from this quarter (keep code)
# - Delete tasks, projects, evaluations
# - Keep employee/user records
# - Keep GitHub history (external)

# 3. Create new project for next quarter
```
...
2. **Create System User**:
   ...
   - Email: firstname.lastname@artemis-lab.local
   - Name: Full name
   - Groups: Select Intern + Development group
   - Save
...
- Give interns admin rights (use groups)
...
- [ ] Database created (`artemis_lab`)
- [ ] Admin user login successful

### **Day 2: Modules & Configuration**
- [ ] 5 mandatory modules installed
- [ ] artemis_custom module installed
- [ ] 9 user groups created
- [ ] CEO user created & logged in
- [ ] Project Manager user created

### **Day 3: Data Setup**
- [ ] 10-15 interns added as employees
- [ ] All interns have system users
- [ ] All interns assigned skills
- [ ] All interns assigned batches
...
- [ ] Intern group has limited access
- [ ] Tech Lead can review tasks
- [ ] Project Manager can assign tasks
- [ ] CEO can create evaluations
- [ ] Admin has full access
- [ ] Test permissions for each role

### **Day 7: Training & Documentation**
- [ ] CEO trained on system
- [ ] Interns given access & passwords
...
# Manual backup
docker exec artemis_postgres pg_dump -U odoo postgres > test.sql
...
- **System Admin**: admin@artemis-lab.local
- **Artemis GitHub**: https://github.com/artemis-lab/
- **Odoo Documentation**: https://www.odoo.com/documentation/17.0/

---

**This setup is production-ready for professional deployment, enterprise requirements, and employee portfolios.**
